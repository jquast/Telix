...
class TestServerTypePresets:
    ...
    def test_gmcp_modules_extra_injects_modules_for_telnet(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        modules_value = sys.argv[modules_idx + 1]
        assert "room.writtenmap 1" in modules_value
        assert "room 1" in modules_value
        assert "char 1" in modules_value

    def test_gmcp_modules_extra_not_injected_when_gmcp_modules_present(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """--gmcp-modules-extra is ignored when --gmcp-modules is already set."""
        monkeypatch.setattr(
        ...
        assert "room.writtenmap 1" not in sys.argv
