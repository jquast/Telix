...
def _parse_bracket_exits(text: str) -> dict[str, str]:
    ...
    Returns a dict of {direction: "1"} for each exit found in the
    bracket-enclosed, comma-separated list of abbreviated directions.
    """
    exits: dict[str, str] = {}
...
_RE_EXIT_TEXT = re.compile(
    r'There are \w+ obvious exits?: (.+?)(?:\.\s*)?$',
...
async def _watch_exits_task(ctx: ScriptContext) -> None:
    """Background task: watch room description text for exit patterns and
    populate the room graph.

    Some Discworld areas (e.g. Ephebe farmland) send exits only in room
    description text rather than in GMCP Room.Writtenmap prose. This
    catches 'There are N obvious exits: dir1, dir2 and dir3.' patterns
    in any server output.
    """
    while True:
        ...
        if exits:
            num = ctx._ctx.room.current
            if num:
                log.debug("discworld_rooms: text-exits for %s: %s",
                           num[:12], exits)
                ctx.update_room({"identifier": num, "exits": exits})
...
def _mark_room_seen(room_id: str) -> bool:
    """Mark a room as seen. Returns True if the room was already seen."""
...
async def _read_signs_task(ctx: ScriptContext) -> None:
    ...
    Only fires once per room change — rooms get re-read only after at least
    100 other rooms have been visited.
    """
    while True:
...
async def run(ctx: ScriptContext) -> None:
    """Register a Room.Writtenmap callback, start the text-based exit
    watcher and sign reader, register area name mappings, and keep the
    script alive."""
