...
Module map::
    ...
    ├── client_repl_commands.py /slash command dispatch
...
telnetlib3 never imports from telix.  telix can evolve ``SessionContext``
freely because it inherits from ``TelnetSessionContext``.

XDG config and data
-------------------

All persistent state lives under standard XDG directories:

- ``~/.config/telix/`` -- autoreplies.json, macros.json, highlights.json,
  sessions.json
- ``~/.local/share/telix/`` -- per-session history, chat logs, room graphs
